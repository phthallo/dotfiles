...
Scope {
    ...
    //
    // Left off the bar strip on purpose: the bar and this backdrop are both
    // WlrLayer.Top, so which one actually catches a click over the bar is
    // down to compositor stacking order, not anything this file controls -
    // the bell's own click to close could land on either one. Starting the
    // backdrop below the bar removes the ambiguity: a click on the bar
    // always reaches the bar.
    PanelWindow {
    ...
    PanelWindow {
        ...
        visible: Notifications.panelOpen
        onVisibleChanged: {
            if (visible) { show.restart(); escScope.forceActiveFocus(); refocusCursor.running = true; }
            // WlrKeyboardFocus.OnDemand can lose the surface to the
            // compositor without our own click-handlers ever firing (focus
            // stolen elsewhere, an output going away). If that happens,
            // Notifications.panelOpen is left stuck true - the loader has
            // already unloaded, so a visible flip does nothing, and the
            // bell's next click just flips panelOpen from true back to
            // false without reopening anything: it takes two more clicks to
            // recover. Syncing back the moment the surface actually
            // disappears keeps that one variable honest.
            else if (Notifications.panelOpen)
        ...
        Component.onCompleted: if (visible) { show.restart(); escScope.forceActiveFocus(); }
        ...
        FocusScope {
            ...
            anchors.fill: parent
            focus: true
            Keys.onEscapePressed: Notifications.panelOpen = false
    ...
    // focus until the pointer physically moves, so the first click on the
    // just-opened panel (including a click meant to close it) is silently
    // dropped. hyprwm/Hyprland#4882, still open. Re-issuing the cursor's own
    // position is a no-op visually but fires the motion event Hyprland
    // needs to hand focus to the new surface.
    Process {
        id: refocusCursor
        command: ["sh", "-c",
