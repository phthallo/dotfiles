...
update_conf_path() {
  ...
  # Anchor on start-of-line + optional whitespace so commented-out lines such as
  # "#path = screenshot" are left alone - without it every path line in the
  # block gets rewritten, including the ones kept around as documentation.
  sed -i "/\\(background\\|wallpaper\\) {/,/}/{s|^\\([[:space:]]*\\)path = .*|\\1path = ${wallpaper_path}|}" "$conf"
}
